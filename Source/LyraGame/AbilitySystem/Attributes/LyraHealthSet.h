...
public:
	...
	ATTRIBUTE_ACCESSORS(ULyraHealthSet, Mana);
	ATTRIBUTE_ACCESSORS(ULyraHealthSet, MaxMana);
...
protected:
	...
	UFUNCTION()
	UE_API void OnRep_Mana(const FGameplayAttributeData& OldValue);

	UFUNCTION()
	UE_API void OnRep_MaxMana(const FGameplayAttributeData& OldValue);
...
private:
	...
	UPROPERTY(BlueprintReadOnly, ReplicatedUsing = OnRep_MaxHealth, Category = "Lyra|Mana", Meta = (AllowPrivateAccess = true))
	...
	UPROPERTY(BlueprintReadOnly, Replicated, Category = "Lyra|Mana", Meta = (AllowPrivateAccess = true))
	FGameplayAttributeData Manaling;
};
