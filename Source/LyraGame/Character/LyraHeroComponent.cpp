...
void ULyraHeroComponent::AddExtraAllowedTags(const FGameplayTagContainer& SourceTags)
{
	for (const FGameplayTag& Tag : SourceTags)
	{
		ExtraAllowedTags.AddStack(Tag, 1);
	}
}

void ULyraHeroComponent::RemoveExtraAllowedTags(const FGameplayTagContainer& SourceTags)
{
	for (const FGameplayTag& Tag : SourceTags)
	{
		ExtraAllowedTags.RemoveStack(Tag, 1);
	}
}

bool ULyraHeroComponent::DoesAbilityPassExtraAllowedTagsCheck(const FGameplayTagContainer& SourceTags)
{
	for (const FGameplayTag& Tag : SourceTags)
	{
		if (!ExtraAllowedTags.ContainsTag(Tag))
		{
			return false;
		}
	}
